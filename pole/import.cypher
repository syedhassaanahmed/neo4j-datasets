...
USING PERIODIC COMMIT LOAD CSV FROM "https://raw.githubusercontent.com/jbarrasa/datasets/master/safeguarding/vulnerable_people.csv" AS row
...
USING PERIODIC COMMIT LOAD CSV FROM "https://raw.githubusercontent.com/jbarrasa/datasets/master/safeguarding/FAMILY_DATA.csv" AS row
MATCH (p1:Person {nhs_no: row[0]}),(p2:Person {nhs_no: row[1]})
CREATE (p1)-[:FAMILY_REL{rel_type: row[2]}]->(p2)
MERGE (p1)-[:KNOWS]-(p2);

// Load data from social networks
USING PERIODIC COMMIT LOAD CSV FROM "https://raw.githubusercontent.com/jbarrasa/datasets/master/safeguarding/SOCIAL_NETWORKS.csv" AS row
...
USING PERIODIC COMMIT LOAD CSV FROM "https://raw.githubusercontent.com/jbarrasa/datasets/master/safeguarding/SOCIAL_WORKER_REPORT_EVENTS.csv" AS row
...
// Load police reports
USING PERIODIC COMMIT LOAD CSV FROM "https://raw.githubusercontent.com/jbarrasa/datasets/master/safeguarding/POLICE_REPORT.csv" AS row
...
USING PERIODIC COMMIT LOAD CSV FROM "https://raw.githubusercontent.com/jbarrasa/datasets/master/safeguarding/CDRs_2015.csv" AS row
